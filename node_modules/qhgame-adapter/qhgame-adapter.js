window.GameGlobal = {};
window.canvas = qh.createCanvas();
// window.canvas.width = 640;
// window.canvas.height = 1136;
...
qh.setPreferredFramesPerSecond = () => { }
...
//暂不支持开放数据域,避免找不到错误
qh.getOpenDataContext = () => {
    console.error("暂不支持wx.getOpenDataContext");
};

window.wx = qh;
window.sharedCanvas = qh.createCanvas();
qh.env = {
    USER_DATA_PATH: 'wxfile://'
}

var touchStartCallbacks = [];
qh.onTouchStart = (callback) => {
    window.canvas.addEventListener("touchstart", callback);
    touchStartCallbacks.push(callback);
}
var touchMoveCallbacks = [];
qh.onTouchMove = (callback) => {
    window.canvas.addEventListener("touchmove", callback);
    touchMoveCallbacks.push(callback);
}
var touchEndCallbacks = [];
qh.onTouchEnd = (callback) => {
    window.canvas.addEventListener("touchend", callback);
    touchEndCallbacks.push(callback);
}
qh.onTouchCancel = (callback) => {

}

qh.offTouchStart = (callback) => {
    if (!callback) {
        for (let func of touchStartCallbacks) {
            window.canvas.removeEventListener("touchstart", func);
        }
    } else {
        window.canvas.removeEventListener("touchstart", callback);
    }
}

qh.offTouchMove = (callback) => {
    if (!callback) {
        for (let func of touchMoveCallbacks) {
            window.canvas.removeEventListener("touchmove", func);
        }
    } else {
        window.canvas.removeEventListener("touchmove", callback);
    }
}

qh.offTouchEnd = (callback) => {
    if (!callback) {
        for (let func of touchEndCallbacks) {
            window.canvas.removeEventListener("touchend", func);
        }
    } else {
        window.canvas.removeEventListener("touchend", callback);
    }
}


qh.createInnerAudioContextOld = qh.createInnerAudioContext;
qh.createInnerAudioContext = () => {
    console.log("qh.createInnerAudioContext!");
    const innerAudioContext = qh.createInnerAudioContextOld();
    //canplayer
    const canPlayerList = [];
    const canPlayCheck = (event) => {
        for (const func of canPlayerList) {
            func.apply(func, event);
        }
    }
    innerAudioContext.addEventListener("canplay", function (event) {
        canPlayCheck(event);
    });
    innerAudioContext.onCanplay = (callback) => {
        console.log(innerAudioContext);
        if (canPlayerList.indexOf(callback) < 0) {
            canPlayerList.push(callback);
        }
    }
    innerAudioContext.offCanplay = (callback) => {
        const index = canPlayerList.indexOf(callback);
        if (index >= 0) {
            canPlayerList.splice(index, 1)
        }
    }

    // onPlay
    const onPlayList = [];
    const onPlayCheck = (event) => {
        for (const func of onPlayList) {
            func.apply(func, event);
        }
    }
    innerAudioContext.addEventListener("onplay", function (event) {
        onPlayCheck(event);
    });
    innerAudioContext.onPlay = (callback) => {
        console.log(innerAudioContext);
        if (onPlayList.indexOf(callback) < 0) {
            onPlayList.push(callback);
        }
    }
    innerAudioContext.offPlay = (callback) => {
        const index = onPlayList.indexOf(callback);
        if (index >= 0) {
            onPlayList.splice(index, 1)
        }
    }
    ...
    // onPause
    const onPauseList = [];
    const onPauseCheck = (event) => {
        for (const func of onPauseList) {
            func.apply(func, event);
        }
    }
    innerAudioContext.addEventListener("onpause", function (event) {
        onPauseCheck(event);
    });
    innerAudioContext.onPause = (callback) => {
        console.log(innerAudioContext);
        if (onPauseList.indexOf(callback) < 0) {
            onPauseList.push(callback);
        }
    }
    innerAudioContext.offPause = (callback) => {
        const index = onPauseList.indexOf(callback);
        if (index >= 0) {
            onPauseList.splice(index, 1)
        }
    }
    ...
    // onError
    const onErrorList = [];
    const onErrorCheck = (event) => {
        for (const func of onErrorList) {
            func.apply(func, event);
        }
    }
    innerAudioContext.addEventListener("onerror", function (event) {
        onErrorCheck(event);
    });
    // onError 
    innerAudioContext.onError = (callback) => {
        if (onErrorList.indexOf(callback) < 0) {
    ...
    innerAudioContext.offError = (callback) => {
        const index = onErrorList.indexOf(callback);
        if (index >= 0) {
            onErrorList.splice(index, 1);
        }
    }


    const onEndedList = [];
    const onEndedCheck = (event) => {
        for (const func of onEndedList) {
            func.apply(func, event);
        }
    }
    innerAudioContext.addEventListener("ended", function (event) {
        onEndedCheck(event);
    });
    // onEnded
    innerAudioContext.onEnded = (callback) => {
        if (onEndedList.indexOf(callback) < 0) {
            onEndedList.push(callback);
        }
    }

    // offEnded
    innerAudioContext.offEnded = (callback) => {
        const index = onEndedList.indexOf(callback);
        if (index >= 0) {
            onEndedList.splice(index, 1);
        }
    }
    ...
    innerAudioContext.seek = (position) => {
        ...
        innerAudioContext.currentTime = position;
    }
    return innerAudioContext;
}


// function getBufferFromBlob(data) {
...
// window.getBufferFromBlob = getBufferFromBlob;



// /**根据消息类型，处理socket onMessage的数据 */
// function socketOnMessageHandler(res, callback) {
//     let data = res.data;
//     if (typeof data == "string") {
//         callback(res);
//     } else if (data.size != undefined && data.type != undefined) { //data是Blob
//         getBufferFromBlob(res.data).then((data) => {
//             callback(data);
//         }).catch((e) => {
//             console.error(e);
//         });
//     } else if (data.byteLength != undefined) { //data是arrayBuffer
//         callback(res);
//     }
// }

// wx.onSocketMessageOld = wx.onSocketMessage;

// wx.onSocketMessage = (callback) => {
//     wx.onSocketMessageOld((res) => {
//         socketOnMessageHandler(res, callback);
//     });
// }

wx.connectSocketOld = wx.connectSocket;
wx.connectSocket = (data) => {
    let result = wx.connectSocketOld(data);
    if (result && result.socket && result.socket.binaryType && result.socket.binaryType != "arraybuffer") {
        result.socket.binaryType = "arraybuffer";
    }
    return result;
}


wx.requestOld = wx.request;


wx.request = (data) => {
    const oldSuccess = data.success;
    const success = (res) => {
        let data = res.data;
        try {
...
(function (modules) { // webpackBootstrap
    ...
    ([
        ...
        (function (module, exports, __webpack_require__) {
            ...
            window.document.createElementOld = window.document.createElement;
            window.document.getElementByIdOld = window.document.getElementById;
            var document = {
                // readyState: 'complete',
                ...
                createElement: function createElement(tagName) {
                    ...
                    if (tagName === 'canvas' || tagName == "style" || tagName == "div" || tagName === 'img') {
                        // return new _Canvas2.default();
                        return window.document.createElementOld(tagName);
                ...
                getElementById: function getElementById(id) {
                    ...
                    return window.document.getElementByIdOld(id);
                },
            ...
            for (let key in document) {
                window.document[key] = document[key];
            }
            var head = new _HTMLElement2.default('head');
            window.document.appendChild = head.appendChild;
